...
function initWidget(){
		...
		if(typeof(jQuery) == 'undefined' || parseFloat(jQuery.fn.jquery) < REQUIRED_JQUERY){ // jQuery has not been loaded
				...
				if(!widget_loaded){
						...
						$(document).on('change','label[for=year] #year',function(){
						...
						$(document).on('click','#lookup_submit',function(){
...
function loadConfigurator(){
		...
		if(jQuery('#configurator').length > 0){
				...
				$(document).on('#configurator .mini','click',function(){
				...
				$(document).on('#configurator ul.tabs li','click',function(){
				...
				$(document).on('#configurator .hitchTabs a','click',function(){
				...
				$(document).on('#configurator .imageTab_content img','click',function(){
						var imgSrc = jQuery(this).attr('src');
						imgSrc = imgSrc.replace('300x225','1024x768');
						window.open(imgSrc,'','resizeable=yes;toolbar=no;status=no');
				});

				$(document).on('#configurator .prodImg','click',function(){
